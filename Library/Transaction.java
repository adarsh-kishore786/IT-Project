...
public class Transaction {
    ...
    ArrayList<Boolean> isReturned= new ArrayList<Boolean>(); //So it will have isReturned one to one map to borrowedBooks
    ...
    private boolean payment(double amount) {
        ...
        //About the two warnings, it's because I just used those variables for input, kept it that way so that we can update our program
        do
        {
    ...
    public double returnBookTransaction(int index)
    {
        ...
        if (statusPayment==true)
        {
            ...
            ++newNumberCopies;
            borrowedBooks.get(index).setNumCopies(newNumberCopies); // I am a little doubtful about this, need to test with data, does it update for the book as such or just update and keep in my array list? Is it synchronized basically?
